var Artist = function(screenWidth,screenHeight){
  ...
  this.drawImage = function(image, x, y, width, height, srcX = 0, srcY = 0, srcWidth = image.width, srcHeight = image.height){
  	if(image.ready === true){
      this.brush.drawImage(image, srcX, srcY, srcWidth, srcHeight, x, y, width, height);
  	}
  }
  ...
  this.drawLine = function(x1,y1,x2,y2,color, size= 3){
    this.brush.beginPath();
    this.brush.strokeStyle = color;
    this.brush.lineWidth = size;
    this.brush.moveTo(x1,y1);
  	this.brush.lineTo(x2,y2);
  	this.brush.stroke();
    this.brush.closePath();
  }
  this.drawDottedLine = function(x1,y1,x2,y2,color, size= 3, dash = [5]){
    this.brush.beginPath();
    this.brush.strokeStyle = color;
    this.brush.lineWidth = size;
    this.brush.setLineDash(dash); // set line dash
    this.brush.moveTo(x1,y1);
    this.brush.lineTo(x2,y2);
    this.brush.stroke();
    this.brush.closePath();
    this.brush.setLineDash([0]); // reset line dash
  }

  this.drawFunc = function(x, y, X, func, scale = 1, color = "black"){
    this.brush.beginPath();
    this.brush.strokeStyle = color;
    this.brush.lineWidth = 5;
    this.brush.moveTo(x + X[0] * scale, y + func(X[0]) * scale);
    for(let i = 1; i < X.length; i++){
      this.brush.lineTo(x + X[i] * scale, y + func(X[i]) * scale);
    }
    this.brush.stroke();
    this.brush.closePath();
  }
